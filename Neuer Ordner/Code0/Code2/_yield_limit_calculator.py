...
def _yield_limit_calculator(df_all, lambda_dict):
    ...
    for key,value in lambda_dict.items():
        if value[0] >= -2 and value[0] < 0:  # negative lambda
            ...
            if len(df_all[df_all[key] != 0][key][177:]) < 50:
                ...
                boxcox_proper_with_lamda = np.power(1 - a*0.01, round(value[0], 3))
                ...
                #print('*************************************' )
                #print("Bin limit of {}".format(key), str(100 - round(inverse_trans_bin_limit * 100, 5)) + '%')
                #print('*************************************' )
                ##print(boxcox_proper_with_lamda)
                calculated_yield_limit = 100 - round(inverse_trans_bin_limit * 100, 5)
                return calculated_yield_limit
            ...
            if len(df_all[df_all[key] != 0][key][177:]) < 50:
                ...
                # a = yield list
                a = df_all[df_all[key] != 0][key][-50:]
                boxcox_proper_with_lamda = np.log(1 - a*0.01)
                #boxcox_proper_with_lamda = stats.boxcox(a, round(value[0], 3))
                #plt.hist(boxcox_proper_with_lamda, bins = 25,  color = 'blue');
                #####################
                q_1 = np.percentile(boxcox_proper_with_lamda,25)
